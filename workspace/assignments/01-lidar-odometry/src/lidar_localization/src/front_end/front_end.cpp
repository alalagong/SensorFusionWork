...
bool FrontEnd::InitRegistration(std::shared_ptr<RegistrationInterface>& registration_ptr, const YAML::Node& config_node) {
    ...
    } else if (registration_method == "ClosedICP"){
        registration_ptr = std::make_shared<ClosedICPRegistration>(config_node[registration_method]);
    } else if (registration_method == "CustomNDT"){
        registration_ptr = std::make_shared<CustomNDTRegistration>(config_node[registration_method]);
    }
    /*
...
bool FrontEnd::UpdateWithNewFrame(const Frame& new_key_frame) {
    ...
    // 保证local和keyframe销毁后，点云释放掉，否则一直在global里占用内存
