...
class CustomNDTRegistration: public RegistrationInterface {
  public:
    CustomNDTRegistration(const YAML::Node& node);
    CustomNDTRegistration(float res, float step_size, float trans_eps, int max_iter);
  ...
  private:
    std::shared_ptr<NormalDistributionsTransform<CloudData::POINT, CloudData::POINT>> ndt_ptr_;
};
